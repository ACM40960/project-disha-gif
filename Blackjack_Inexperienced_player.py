# Blackjack simulator: inexperienced player logic
...
# Function to simulate a single blackjack game with inexperienced player logic
def play_blackjack():
    ...
    # Inexperienced player's turn: Keep hitting until the hand value is at least 17
    while True:
...
def simulate_inexperienced_blackjack(num_simulations):
    ...
    plt.title(f'Blackjack Simulation Results (Inexperienced logic) ({num_simulations} simulations)')
...
def run_simulation():
    ...
    simulate_inexperienced_blackjack(num_simulations)
    
# Run the simulation 100000 times and store the results for the final plot
def run_simulation_100000_times_inexperienced_blackjack():
    num_simulations = 100000
    results = {'wins': 0, 'losses': 0, 'ties': 0}
    
    # Run the simulation for 100000 times and record the outcomes
    for _ in range(num_simulations):
        result = play_blackjack()
        if result == 1:
            results['wins'] += 1
